...
**A secure, serverless Instagram-style application demonstrating Cognito authentication, least-privilege IAM, event-driven image processing, and cloud-native deployment on AWS.**
...
![Architecture Diagram](./screenshots/frames-architecture.jpg)
...
This created a security risk because long-lived credentials should not be exposed to browser-based applications.

The access architecture was redesigned using **Amazon Cognito Identity Pools**:
...
The IAM role was restricted to only the resources and operations required by Frames:
...
The original credential references were removed from the application and the complete authentication, upload, posting, liking, and deletion flows were re-tested successfully.
...
Performance was measured against the deployed application using real end-to-end upload tests.

**Measured flow:**
`S3 upload → DynamoDB post creation → feed refresh`
...
* Production application hosted on AWS
